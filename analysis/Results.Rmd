...
df1 <- df %>%
  ...
  pivot_longer(
    cols = 
      -c(record_id, country, region, region_exact, HBC_ANY, HBC_TB, HBC_TB_HIV, HBC_DR_TB, `Level of integration`, `Income Level`, `HIV Prevalence`, `LTBI testing`),
    names_to = "variable",
    values_to = "category")  %>% 
  mutate(
...
theme_gtsummary_journal(journal="jama")
check <- df |> 
  dplyr::select(ltbi_ligibility.factor, screen_oth.factor)
...
# Overall
# ov <- df7 %>% 
#   dplyr::select(record_id,`Cough`:Other) %>% 
...
# 
# reg_af <- df7 %>% 
#   dplyr::select(record_id, region , `Cough`:Other ) %>% 
#   filter(region == "Africa") %>% 
#   pivot_longer(cols = -c(record_id, region), names_to = "TB symptom screening") %>% 
#   dplyr::select(-record_id, -region) %>% 
#   tbl_summary(by = value, percent = "row") %>% 
...
#   dplyr::select(-stat_3_2, -stat_2_2) %>% 
...
```{r, include=FALSE}
check <- df7 %>% 
  dplyr::select(record_id, `LTBI testing to check eligibility of PLHIV to receive TPT`, `Which LTBI testing is offered?`) 
...
check2 <- df %>% dplyr::select(record_id, region, name, ltbi_testing___0, ltbi_ligibility.factor, tbhiv_integration.factor) %>% 
...
```{r}
check <- df2 %>% filter(variable == "Medication availability")
...
inh/rif
```{r}
table(df5_model$RIF, df5_model$region)
table(df5_model$INH, df5_model$`Income Level`)
tabyl(df5_model$INH)
tabyl(df5_model$RIF)
...
df5_model %>% group_by(HBC_ANY) %>%
  summarize(
    total = n(),
    count = sum(`3-month isoniazid-rifampicin (3HR)` %in% c("Adults", "Adults & Children", "Children")),
    proportion = round(count / total, 2)) %>% gtsummary::tbl_uvregression(
      method = glm,
      include = c(HBC_ANY),
      y = cbind(count, total - count),
...
high burden comparison

```{r}
# I use logistic regression to compare the odds of offering TPT to household contacts in high burden countries compared to non-high burden countries
hbc_tpt<- df1 %>%
  group_by(HBC_ANY, who_group) %>% 
   summarize(sum = sum(category == "Yes"),
             n = n(),
    prop = round(sum(category == "Yes") / n(),2)) 

glm(cbind(sum, n - sum) ~ HBC_ANY, data = hbc_household %>% filter(who_group == "Household contacts"), family = binomial()) %>% 
  tbl_regression(exponentiate = TRUE, pvalue_fun = ~style_sigfig(., digits = 2), add_estimate_to_reference_rows = TRUE)

glm(cbind(sum, n - sum) ~ HBC_ANY, data = hbc_household %>% filter(who_group == "Other people at risk"), family = binomial()) %>% 
  tbl_regression(exponentiate = TRUE, pvalue_fun = ~style_sigfig(., digits = 2), add_estimate_to_reference_rows = TRUE)
```
...
```{r}
hiv_comp <- df1 %>%
  mutate(`HIV Prevalence` =  case_when(`HIV Prevalence` %in% c("Low (<1%)", "Middle (1-5%)") ~ "Low", TRUE ~ "High")) %>%
  group_by(`HIV Prevalence`, who_group) %>% 
   summarize(sum = sum(category == "Yes"),
             n = n(),
    prop = round(sum(category == "Yes") / n(),2)) %>% 
  filter(who_group == "Household contacts")

x_values <- as.numeric(hiv_comp$sum)
n_values <- as.numeric(hiv_comp$n)
prop_test_result <- prop.test(x = x_values, n = n_values)
